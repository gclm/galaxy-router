...
- [ ] Admin API RESTful 规范化（优先级：P1）— 路径/方法对齐 RESTful 约定，前后端联动
- [ ] 错误类型统一迁移到 `src/error/`（优先级：P2）— `AppError` + `ProxyError`
- [ ] 使用 `data/galaxy.db` 的 E2E 测试（优先级：P2）— 补充真实数据端到端验证
- [ ] 前端管理后台 UI 完善（优先级：P2）

## 已知技术债

- `architecture/ARCHITECTURE.md` Admin API RESTful 不规范段 — 建议走 refactor SOP
- `relay/error.rs` + `api/response.rs` 错误类型分散 — 建议走 refactor SOP
