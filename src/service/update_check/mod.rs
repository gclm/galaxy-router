//! 版本更新检查 service：对比当前版本与 GitHub latest release，判断是否有新版本。
//!
//! 从 api/handlers/admin/update_check.rs 归位（D1）。仅取"检查"（不自动更新）。
//! check/fetch 返回 String 错误，handler 转 ApiError（service 不耦合 axum）。
//! 代理复用 settings 的 proxy.url；GitHub 仓库从 settings.github.repo（fork 可自定义）。
//! 结果服务端短期缓存，超时及时降级。
...
impl UpdateCheckContext {
    /// 生产：从 settings 读取代理 + GitHub 仓库 + 镜像，构建客户端（router 启动时调用）。
...
async fn fetch_and_compare(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("检查更新失败：无法连接 GitHub（{e}）"))?;
    ...
    let release: GithubRelease = resp
        .json()
        .await
        .map_err(|e| format!("检查更新失败：解析响应失败（{e}）"))?;
...
mod tests {
    ...
    fn has_update_true_when_latest_newer() {
        ...
        assert!(has_update("1.0.2", "v1.0.3"));
    }

    #[test]
    fn has_update_false_when_equal() {
        assert!(!has_update("1.0.2", "1.0.2"));
        assert!(!has_update("1.0.2+homebrew.x", "v1.0.2"));
    }
    ...
    async fn call_check(state: &UpdateCheckContext) -> UpdateCheckResponse {
        state.check().await.expect("check 应成功")
    }

    #[tokio::test]
    async fn detects_new_version() {
        ...
        let resp = call_check(&state).await;
        assert!(resp.has_update);
        assert_eq!(resp.latest_version, "99.0.0");
        assert_eq!(resp.release_notes, "release notes");
    }

    #[tokio::test]
    async fn no_update_when_latest_older() {
        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/repos/test/repo/releases/latest"))
    ...
    async fn cache_hit_skips_refetch() {
        ...
        let r1 = call_check(&state).await;
        let r2 = call_check(&state).await;
        assert_eq!(r1.latest_version, r2.latest_version);
    }
    ...
    async fn returns_error_when_github_fails() {
        ...
        assert!(state.check().await.is_err(), "GitHub 5xx 应返回错误而非 panic");
    }
