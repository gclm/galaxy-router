//! 端点探测 service：对端点发流式探测请求，返回连通性 + 思维链诊断（D2 归位）。
//!
//! 从 api/handlers/admin/channels/probe.rs 迁入。零依赖（仅 reqwest::Client + domain::channel）。
//! 多协议增量消费 + 思维链检测在此；handler 只 map String → ApiError。
...
/// 端点测试请求（不依赖已保存渠道：base_url/headers/api_key/model 都从请求体传，新增/已存渠道都可测）
...
impl ChannelProbe {
    ...
    /// 探测：发流式请求 + 增量消费，返回连通性 + 思维链诊断（per-endpoint，不合并）。
    /// Err 仅用于协议不支持（请求构造失败）；网络/上游失败进 Response.error。
    pub async fn test_endpoint(
        &self,
        req: TestEndpointRequest,
    ) -> Result<TestEndpointResponse, String> {
        let endpoint_type = parse_protocol(&req.endpoint_type)
            .ok_or_else(|| format!("不支持的端点类型: {}", req.endpoint_type))?;

        let (body, _upstream_path) = build_probe_payload(&endpoint_type, &req.model)
            .ok_or_else(|| format!("端点类型 {} 不支持测试", req.endpoint_type))?;

        // 用请求体参数构造临时 endpoint（不依赖已保存渠道）
    ...
    match protocol {
        ...
        EndpointType::Anthropic => Some((
            serde_json::json!({
                "model": model,
                "messages": [{"role": "user", "content": DETECT_PROMPT}],
                // Anthropic 规则：budget_tokens 必须 < max_tokens
                "max_tokens": 4096,
...
mod tests {
    ...
    fn parse_protocol_round_trips_known_strings() {
        assert_eq!(parse_protocol("openai_chat"), Some(EndpointType::OpenAiChat));
        assert_eq!(
